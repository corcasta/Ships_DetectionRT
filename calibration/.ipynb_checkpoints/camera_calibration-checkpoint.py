...
def calibrate(pattern_size, visualize=False):
    ...
    mtx_df.to_string("cam_intrinsics.txt", index=False, header=False)

    #data = pd.DataFrame(newcameramtx)
    #data.to_string("newcam_intrinsics.txt", index=False, header=False)
    
    dist_df = pd.DataFrame(dist)
    dist_df.to_string("dist_coefficients.txt", index=False, header=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--corners", 
                        help="Provide only 2 integers representing the number of inner corners in chessboard (vertical corners, horizontal corners)",
                        nargs='+', 
                        type=int)
